...
def my_main(_run, _config, _log):
    ...
    #config["unique_token"] = f"{config['exp_name']}_{map_name}_{datetime.datetime.now()}_{config['remark']}".replace(" ", "_")
...
if __name__ == '__main__':
    ...
    def _get_argv_config(params):
        ...
        for _i, _v in enumerate(params):
            item = _v.split("=")[0]
            if item[:2] == "--" and item[2:] not in ["env-config", "config"]:
                config_v = _v.split("=")[1]
                try:
    ...
    params_dict = _get_argv_config(params)
    config_dic = recursive_dict_update(config_dict, params_dict)
