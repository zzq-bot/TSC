...
class HeuristicAgent(Agent):
    ...
    def _center_of_players(self, players):
        coords = np.array([player.position for player in players if player])
        return np.rint(coords.mean(axis=0))

    def _furthest_player(self, players, pos):
        coords = [player.position for player in players if player]
        if len(coords) != 0:
    ...
    def _eligible_furthest_player(self, players, level, pos):
        coords = [player.position for player in players if player]
        levels = [player.level for player in players if player]
    ...
    def real_step(self, obs):
        #assert isinstance(obs, torch.Tensor), print(obs, type(obs))
        #assert len(obs.shape) == 1, print(obs.shape)
        np_acs = self._step(obs)
        ret = torch.LongTensor([int(np_acs.value)])
        return ret                 
...
class H4(HeuristicAgent):
    ...
    def step(self, obs):
        ...
        players_center = self._center_of_players(obs.players)
        players_sum_level = sum([a.level for a in obs.players if a])
    ...
    def step(self, obs):
        ...
        try:
            ...
            for player in obs.players:
                if player and player.position == leader_loc:
